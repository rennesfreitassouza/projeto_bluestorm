...
from bluestorm_api.sqlite_db import select_patients_data
from bluestorm_api.sqlite_db import select_pharmacies_data, select_trasactions_information
from bluestorm_api.auth_functions import auth, token_required
from bluestorm_api.extra_args_para_busca import get_patients_args, get_pharmacies_args
from bluestorm_api.extra_args_para_busca import get_transactions_args
...
def pharmacies_endpoint():
    ...
    if request.method == 'GET':
        nome = get_pharmacies_args(request.args)
        name = sanitize_input(nome)
...
def transactions_endpoint():
    ...
    if request.method == 'GET':
        ...
        json_data = jsonify(select_trasactions_information(
                    pa_first_name=pa_first_name, ph_name=ph_name))
        return json_data
